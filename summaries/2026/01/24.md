...
## 23:03:55
The provided log details changes primarily focused on the **3CAT Smart TV application's build process, platform-specific configurations, image handling, and analytics integration**, spanning from January 17th to January 23rd, 2026.

**File-Specific Updates:**

*   **`build_templates\README.md` (17/1/2026, 10:37:02 and 11:11:00):** This file was initially updated to outline the purpose and structure of the `build_templates` directory, which houses base project templates for Tizen and WebOS Smart TV Apps (DEV, PRE, PRO environments). It details "launcher_app" (minimal web-hosted shell) and "embedded_app" (future fully packaged version) types, along with expected output file naming conventions. A later update on the same day clarified the folder structure under `webos/` to match the `3cat_dev`, `3cat_pre`, `3cat_pro` naming convention already present in `tizen/`.

*   **`build_templates\tizen\README.md` (17/1/2026, 10:37:46):** This README provides specific Tizen CLI commands for packaging (`tizen package`), listing devices (`sdb devices`), and installing (`tizen install`) `.wgt` packages.

*   **`build_templates\tizen\3cat_dev\gateway\js\app.js` (17/1/2026, 10:45:06):** This JavaScript file is configured for the Tizen DEV environment. It initializes a `REMOTE` URL to `https://smartv-dev.3cat.website/index.html`. It checks for network connectivity and redirects to the `REMOTE` URL if connected. If not, it displays an alert, registers 'Exit' and 'Back' keys (VK\_BACK, VK\_EXIT) to exit the application, and sets up a network state change listener to redirect upon connection. A `platformVersion === '3.0'` check is also present.

*   **`build_templates\tizen\3cat_pre\gateway\js\app.js` (17/1/2026, 10:45:19):** Similar to the `_dev` version, but configures the `REMOTE` URL to `https://smartv-pre.3cat.website/index.html` for the Tizen PRE environment. The logic for network check, key registration, and redirection remains identical.

*   **`package-launcher-apps.cjs` (17/1/2026, 10:56:17, 10:57:03 and 22/1/2026, 17:59:52, 18:00:20, 18:01:35, 18:02:56):** This Node.js script is central to packaging launcher applications. It handles CLI arguments (`--platform`, `--env`), resolves app name and version (prompting for binary version `X.X.X`), and manages output directories. It includes functions for directory manipulation (`ensureDir`, `copyRecursive`, `removeDirRecursive`), package listing (`listPackages`), and picking newly generated files (`pickNewOrLatest`).
    *   **WebOS packaging:** It iterates through `720` and `1080` resolutions, copies template directories, updates `appinfo.json` with resolution and version, and executes `ares-package`.
    *   **Tizen packaging:** It copies the template, updates `config.xml` with the binary version, and conditionally prompts for a Tizen security profile (`chooseTizenProfile`) before running `tizen package`.
    *   Multiple minor updates occurred in this file, likely small refinements or formatting changes, as the core logic described above remains consistent across the timestamps. The prompt for Tizen security profile logic for PRO packages is a key feature.

*   **`build_templates\tizen\3cat_dev\config.xml` (17/1/2026, 11:02:30):** This Tizen configuration file defines the app's ID (`i40jdPIQJb.3Cat`), version (`3.4.6`), content source (`index.html`), screen features, metadata (e.g., `prelaunch.support`, `use.network`, `devel.api.version`, `use.preview` endpoint), privileges (e.g., `application.launch`, `internet`), and a `tv-samsung` profile.

*   **`build_templates\tizen\3cat_pre\config.xml` (17/1/2026, 11:03:00):** Similar to the DEV config, but with a different Tizen application ID (`PMVOmA9N8K.3Cat`). All other configurations, including version `3.4.6`, are identical.

*   **`.project` files within `build_templates\tizen\*` and `build_templates\webos\*` (17/1/2026, 11:16:08 to 11:34:16):** These XML files define project settings for Tizen and WebOS environments, including project names (`3catdev`, `3cat_pre`, `3catDev`, `3catPre`, `3cat_dev`), build commands (e.g., `json.validation.builder`, `org.tizen.web.project.builder.WebBuilder`, `com.webos.ide.nature.WebOSBuilder`), and natures (e.g., `WebNature`, `WebOSNature`). There were several changes to the `<name>` tag, indicating adjustments to project display names, and standardizations of names like `3cat_dev` and `3cat_pre`.

*   **`build_templates\webos\3cat_dev\gateway\js\app.js` (17/1/2026, 11:21:52):** This JavaScript file for WebOS DEV environment sets up `REMOTE` URL to `https://smartv-dev.3cat.website/index.html`. It requests system info to determine SDK version, sets the `connectionmanager` service (either `WEBOS_MANAGER` or `PALM_MANAGER` based on SDK version), checks internet connection, and redirects or displays a network error. It also registers a `VK_BACK` key to `webOS.platformBack()`.

*   **`appinfo.json` within `build_templates\webos\*` (17/1/2026, 11:34:24 to 11:35:29):** These JSON files configure WebOS app details. Initial changes standardized `title` (e.g., "3Cat_pre", "3Cat_dev") and `id` ("cat.3cat.webos"). Later, the `id` was updated to a more generic "com.mediapro.3cat" for both DEV and PRE environments. All instances specify `resolution: "1920x1080"`, `version: "2.4.3"`, and other standard metadata.

*   **`build_templates\webos\3cat_pre\gateway\js\app.js` (17/1/2026, 11:36:57 and 11:43:23):** Similar to the DEV version, but configures the `REMOTE` URL to `https://smartv-pre.3cat.website/index.html` for the WebOS PRE environment. A later update added a query parameter `?pr=true` to the `REMOTE` URL for some redirects, likely related to a preproduction flag.

*   **`src\utils\platformUtils.js` (17/1/2026, 12:23:41 to 20/1/2026, 11:56:50):** This utility file manages platform detection and resolution configuration.
    *   It defines `window.AppGlobal` for global app state (actions, platform, versions, player data, etc.).
    *   `getPlatform()` identifies the platform (`browser`, `tizen`, `webos`, `hisense`, `lg`, `hbbtv`) based on the user agent, and for `webos` it injects the `webOSTV.js` library dynamically.
    *   `configurePlatformResolution()` sets `window.AppGlobal.windowResolution`, `screen` dimensions, and applies a `scale` factor (1.5 for 720p, 0.5 for 2160p, 1 for 1080p) to adjust UI.
    *   `getOSVersion()` retrieves the OS version for different platforms.
    *   Multiple commits refined `getPlatform()`:
        *   Initially, it accepted an optional `platformOverride`.
        *   It was updated to check for "Web0S" in addition to "WebOS" in the User Agent.
        *   The "hisense" platform detection was removed.
        *   Later changes simplified the `getPlatform` function to primarily check for `qsPlatform` from URL parameters for override, and if not present, rely on User Agent for platform detection.
        *   A console log `console.log(window.KeyEvent)` was temporarily added and then removed.

*   **`src\App.jsx` (17/1/2026, 12:27:06 to 17/1/2026, 12:28:11 and 17/1/2026, 12:43:32):** This is the main React application component.
    *   It imports numerous components, contexts, and utilities for routing, authentication, data management, and UI.
    *   `InitDevConsole` configures an on-screen debug console.
    *   `InitApp` initializes the platform using `Platform.initPlatform`.
    *   `BuildInfo` component displays various debug information (environment, user agent, app version, build number, platform, OS version, screen/window resolution).
    *   `AppInner` handles application logic like exit/logout modals, navigation, and user management. It uses `BrowserRouter` and `Routes` for defining public and private routes.
    *   The primary change across timestamps in `App.jsx` relates to the `InitApp` function's `platformOverride` parameter: it was initially present, then removed, and later reintroduced, suggesting an evolving strategy for how platform detection can be explicitly overridden, likely for testing purposes. It also removes `qsPlatform` being passed directly to `InitApp`.

*   **`src\platform\platform.js` (17/1/2026, 12:37:07):** This file acts as an abstraction layer for platform-specific functionalities. It imports device-specific modules (Tizen, WebOS, HbbTV, Browser) and provides `initPlatform`, `initKeyboard`, and `launchApp` methods. It also re-exports `window.PlatformUtils.getPlatform`. Key mapping for 'web0s' was added to `launchApp`.

*   **`src\platform\generic.js` (17/1/2026, 12:57:09 and 12:59:31):** This file defines generic keyboard navigation for Smart TVs. `IinitKeyboardNavigation_SmartTV` registers a `keydown` handler to manage navigation (`up`, `down`, `left`, `right`) and `action` (Enter key simulating click). It also handles the 'Back' key for various platforms (PC browser, Samsung Tizen, LG WebOS, Hisense, TIVO), triggering an exit modal or history navigation. The main change was updating key codes for 'Back' and 'Escape' for different platforms (e.g., `0x1cd` to `461` for TIVO, `0xa6` to `166` for VK\_ESCAPE, `0x1b` to `27` for VK\_BACK_SPACE).

*   **`package.json` (20/1/2026, 11:00:55):** This file defines project metadata, dependencies, and a comprehensive set of npm scripts. It includes scripts for version/build/date/env upgrades, local development, base device builds (WebOS, Tizen), SmartTV and HbbTV builds (including hosted webapp builds), previewing builds, linting, and various device packaging commands for "launcher apps" and "embedded apps" (the latter currently marked as TODO). It also includes Google Cloud Storage (`gsutil`) deployment commands for hosted webapps. The version is `1.0.4`, build `328`, and date `2026-01-16 08:40:40`.

*   **`profiles.xml` (21/1/2026, 11:41:12 and 11:41:44):** This Tizen Studio configuration file manages security profiles. The main change was renaming a duplicate "SHTV_PUBLIC" profile entry to "CCMA" and updating its associated certificate paths to refer to the "CCMA" folder, suggesting a new or re-branded signing profile.

*   **`src\libs\img.js` (21/1/2026, 12:24:48 to 21/1/2026, 15:40:18):** This utility file for image processing underwent significant changes.
    *   It defines `IMGPROXY_PRESETS` for responsive image sizing.
    *   `getImgProxyUrl` was initially commented out to return the original `imgUrl` but was then uncommented and enhanced to construct a URL using `window.appConfig.IMGPROXY_URL` if available, handling transparency, and performing a safety check for proxying.
    *   `removeItemsFromArray` filters out images based on `NOT_ALLOWED_FORMATS` (e.g., 'logo', 'triangular').
    *   `getBestImage` and `getBestImageV2` (for landscape/portrait aspect ratios) select the most suitable image from a list based on width, height, aspect ratio, distance, and priority (text/no-text). `getBestImage` was updated to handle `forceFormat` as an array.
    *   `getLogoImage` and `getImageByFormat` retrieve specific image formats.
    *   `generateImageData` and `generateImageDataFromLiveChannelInHome_v2` encapsulate the logic for generating image data for UI components.
    *   A significant change was introduced in `getBestImage` and `getBestImageV2` to conditionally apply `getImgProxyUrl` only if `bestImage.is_external` is *false*, indicating that external images should not be processed by the internal image proxy.
    *   Another change involved an initial implementation for `getImgProxyUrl` that included a safety check (`isSafeForProxy`) but this version was later reverted, going back to a simpler check for `window.appConfig.IMGPROXY_URL`.

*   **`src\components\items\rowItems\portraitItem\index.jsx` (21/1/2026, 12:44:34 and 12:44:45):** This React component renders a portrait-oriented item in a row. It uses `generateImageData` to get the image, registers focus actions, and displays ranking, audio icons, or future date labels. It also handles external content, displaying a platform logo and an external link icon. The core change was a slight adjustment in how `onFocusAction` is triggered for mouse hover.

*   **`src\components\modals\externalContentModal\index.jsx` (21/1/2026, 12:46:58 to 21/1/2026, 13:17:26):** This React component renders a modal for external content. It displays content details, promo text, platform information, and buttons to open external content or continue within the 3Cat app.
    *   Initially, `generateImageData` for the modal's image explicitly requested `modal-portrait`. Later, this was changed to accept an array `['modal-portrait', 'portrait']` to provide fallback options.
    *   There were several minor fixes related to image rendering: initially rendering `img` directly, then introducing a conditional `imgData?.image?.file ? <img .../> : <div className="no-image" />`, then for a short period removing the `img` tag entirely and only rendering `<div className="no-image" />`, then reintroducing `<ImageCropper>` to render the image, and finally ensuring `(!imgData?.image)` is checked for the `no-image` fallback. This sequence suggests debugging and refinement of image display within the modal.

*   **`src\components\modals\externalContentModal\styles.scss` (21/1/2026, 13:36:16 to 21/1/2026, 13:52:29):** This SCSS file styles the external content modal. Updates involved refining image display within `.popupLeft` by removing direct `img` styling and adjusting the `platformLogoContainer` positioning (`position: absolute; bottom: 0;` to `position: relative; top: 544px;`) to correctly place the platform logo after the main image.

*   **`src\components\imageCropper\index.jsx` (21/1/2026, 14:06:50 to 21/1/2026, 14:27:28):** This React component handles image cropping and display. It calculates aspect ratios and positions images within a frame.
    *   A `useState` hook `showImage` was added to manage image visibility and handle `onError`.
    *   There were several attempts to refine the `imageAspectRatio` calculation and add guard clauses for missing image data, which were later reverted or simplified, indicating iterative debugging. The primary change was the introduction of a protective check for minimum required image and frame data.

*   **`src\contexts\settingsContext.jsx` (21/1/2026, 15:47:39):** This React context manages application settings. It fetches settings data via `ApiGetSettings`, sets `window.currentGeozone`, `window.AppProfile`, and crucially, `window.appConfig.IMGPROXY_URL`. It also sets `staticPagesJson` for analytics.

*   **`src\contexts\sitecatalystContext.jsx` (23/1/2026, 12:17:56 and 12:18:43 and 13:35:06):** This React context manages Sitecatalyst (Adobe Analytics) tracking.
    *   It defines various variables for tracking (e.g., `OMACCOUNT`, `DEVACCOUNT`, `VISITORID`, `APP_NAME`).
    *   It includes functions to `setStaticPagesJson`, `setAccount`, `clearVars`, `getUserId`, `getDeviceID`, `getURL`, `getFullURL`, `getLoginState`, `getStaticPageVars`, `setPageVars`, `setVideoVars`, and `setCommonVars`.
    *   Key updates included changing `s.eVar23` and `s.prop23` to dynamically report `HbbTV` or `SmartTV` based on `window.AppGlobal.platform`.
    *   Another significant update was the addition of `s.eVar10 = s.prop10 = s.eVar24 = s.prop24 = window.AppGlobal.platform;` in `setCommonVars`, which explicitly sets platform information in additional analytics variables.

**Recurring Elements and Patterns:**

*   **Platform-Specific Logic:** A dominant pattern is the clear separation of logic for different Smart TV platforms (Tizen, WebOS, HbbTV) within `build_templates`, `gateway/js/app.js` files, and `src/utils/platformUtils.js`. This includes distinct API calls, key code handling, and build configurations (`config.xml`, `appinfo.json`).
*   **Environment-Specific Configuration:** Builds are consistently structured for `DEV`, `PRE`, and `PRO` environments, with distinct URLs and potentially different app IDs/names.
*   **Build Automation:** The `package-launcher-apps.cjs` script and `package.json` scripts highlight a robust, automated build and packaging pipeline for creating distributable application bundles (.ipk for WebOS, .wgt for Tizen). This includes versioning, date stamping, and security profile signing.
*   **Image Handling and Optimization:** There is a significant focus on image loading, scaling, and processing, evident in `src/libs/img.js`, `src/components/imageCropper/index.jsx`, and `src/components/modals/externalContentModal/index.jsx`. This involves logic to select the "best" image based on dimensions and format, and to optionally use an `IMGPROXY_URL` for efficient image delivery, with specific considerations for external content not being proxied.
*   **Analytics Integration:** `src/contexts/sitecatalystContext.jsx` shows a comprehensive implementation of Adobe Analytics (Sitecatalyst) tracking, including setting various eVars and props based on user state, device, and page context.
*   **Keyboard Navigation:** `src/platform/generic.js` demonstrates a generic approach to handling keyboard inputs for Smart TV environments, with specific key codes mapped for "Back" and "Enter" actions across different devices.
*   **Global State Management:** The `window.AppGlobal` object in `src/utils/platformUtils.js` serves as a central repository for application-wide settings and actions.
*   **Error Handling and Debugging:** Frequent `console.log` and `console.error` statements are observed, indicating an emphasis on debugging and logging during development.
*   **Modals and User Interaction:** Components like `ExternalContentModal` and the logic for `ExitAppModal` in `App.jsx` illustrate focus on user interaction, particularly around exiting the application or navigating to external content.
*   **Refinement of Image Proxying:** The changes in `src\libs\img.js` regarding `getImgProxyUrl` indicate an iterative process of deciding when and how to apply image proxying, eventually excluding external images from being processed by the internal proxy.
*   **Analytics Platform Differentiation:** The `sitecatalystContext.jsx` updates to `s.eVar23` show an effort to differentiate between "HbbTV" and "SmartTV" platforms within analytics tracking.