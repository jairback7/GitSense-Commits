...
## 22:46:19
The code changes primarily occurred between February 2nd and February 5th, 2026, focusing on two main areas: **Smart TV application logic and UI/UX improvements, especially for kid-mode, and image handling.**

### File-Specific Updates:

*   **`c:\Users\Usuario\Documents\Trabajo\3cat_pro\gateway\js\app.js` (2/2/2026, 14:16:27)**:
    *   This JavaScript file, likely a gateway or initial loader, was updated to determine the `connectionmanager` service based on the WebOS SDK version.
    *   It dynamically sets a `REMOTE` URL (either `smartv-dev.3cat.website` or `smartv-pro.3cat.website`) based on SDK version (SDK < 3.50 uses `smartv-pro`).
    *   It also includes logic to check internet connectivity, redirecting if available or displaying an alert and setting up a `keydown` listener for `VK_BACK` to `webOS.platformBack()` if offline. It also subscribes to network status changes.

*   **`c:\Users\Usuario\Documents\Trabajo\MEDIAPRO\3cat_smarttv\src\components\mediaInfo\infoGroup\index.jsx` (2/2/2026, 16:31:34)**:
    *   **New `EpisodeCount` component**: Displays the number of chapters, with a conditional render based on `episode_count` and `LIVE_REFRESH_INTERVAL` (CCMA-1685).
    *   **New `SeasonsCount` component**: Displays the number of seasons, with localized text for singular/plural.
    *   **`Publication` component commented out**: JIRA CCMA-2104 notes the removal of "A partir del" functionality.
    *   **`renderSeasonOrEpisodeCount` function**: Determines whether to show season or episode counts based on item properties (`is_episode`, `season_count`, `episode_count`).
    *   **`showExpiration` function**: Calculates and displays an expiration date string ("Disponible fins a l'/al [formattedDate]") if `unpublished_on` is within three months.
    *   **Title/Description logic updates**: Enhanced logic to determine the appropriate title and description, including handling `default_episode`, `h2`, `presenter`, `stream?.channel`, `collection`, and `media_type` for live, audio, and thematic content.
    *   **Image handling**: `logoImage` and `channelLogo` now use `getLogoImage` for dynamic logo fetching.
    *   **`shouldShowProgramTitle` and `shouldShowEpisodeTitle` memos**: Refined conditions for displaying program or episode titles based on `logoImage`, `tag_format`, `is_episode`, and `collection`.
    *   **`rating_tags` filtering**: Ensures `rating_tags` have a non-empty label.
    *   **`dotSeparatedItems`**: Updated to remove `showPublication` (related to CCMA-2104).
    *   **External Content Rendering**: Added JSX for `is_external` content, including external platform logos and specific `MediaTag` types (`live`, `white`).
    *   **Menu Detail Title**: `MediaTitle` for `upcoming_event` uses `item.label` and `item.title`.
    *   **Licensed Content Tag**: Displays "Disponible [date]" for `is_licensed === false` content.
    *   **Live Stream Logo**: `MediaTag` for live streams now passes `channelLogo`.
    *   **Logo Image Styling**: Conditional `filter` commented out, suggesting previous styling experiments.

*   **`c:\Users\Usuario\Documents\Trabajo\MEDIAPRO\3cat_smarttv\src\components\mediaInfo\mediaTitle\styles.scss` (2/2/2026, 16:33:36)**:
    *   Updated `font-size` for various `mediaTitle` classes: `title_program`, `live`, `isLive_or_upcoming_event`, and `stream` are now `38px` (from `56px`), while `menu_detail` and `external` are `38px` and `56px` respectively, with `menu_detail` having `font-weight: 700`.

*   **`c:\Users\Usuario\Documents\Trabajo\MEDIAPRO\3cat_smarttv\src\components\items\rowItems\portraitItem\styles.scss` (2/2/2026, 16:39:57)**:
    *   Added styles for `.platform-logo` and `.externalIcon`, suggesting integration of external platform branding on portrait items.
    *   Updated `focused` and `hover` states to use `box-shadow` and `outline` with `$base-color-3cat-red`.

*   **`c:\Users\Usuario\Documents\Trabajo\MEDIAPRO\3cat_smarttv\src\components\keyboard\styles.scss` (2/2/2026, 16:56:08 & 17:09:25)**:
    *   **16:56:08**: Introduced specific styling for different key sizes (`.SMALL`, `.MEDIUM`, `.LARGE`) and icons (`.icon-BACKSPACE`, `.icon-SPACE_BAR`, `.icon-RETURN`) for a custom keyboard component.
    *   **17:09:25**: Added a `:hover` state for `.keyboard-key` with the same `box-shadow` and `border` as the `.focused` state, providing visual feedback on mouse hover.

*   **`c:\Users\Usuario\Documents\Trabajo\MEDIAPRO\3cat_smarttv\src\platform\generic.js` (2/2/2026, 17:08:44 & 5/2/2026, 13:32:40)**:
    *   **2/2/2026, 17:08:44**:
        *   Introduced `window.AppGlobal.KeyMemory` to track the last three key presses for combination actions (e.g., enabling/disabling ads with "123" or player buttons).
        *   Added `PLAYER_KEYS` set to filter key events when the player is not ready.
        *   Modified `onKeyDownHandler` to prevent default behavior and stop propagation for all key events.
        *   Commented out `e.preventDefault()` and `e.stopPropagation()` in the `onKeyDownHandler` but likely meant to keep them.
        *   Added `dispatchNavigationEvent` for 'up' and 'down' when `nav.fixedFocus.isNavigating` is false and `window.isUserModalVisible` is false.
        *   Unified back key handling (`VK_BACK`, `10009` for Tizen, `461` for WebOS, `88` for Hisense, `VK_ESCAPE`, `VK_BACK_SPACE`) to trigger `goBackHistory` and `window.AppGlobal.Actions.openExitAppModal`.
        *   Removed `dispatchRewindEvent()` and `dispatchFastForwardEvent()` calls from platform-specific blocks (WebOS, Tizen, other non-HbbTV platforms).
    *   **5/2/2026, 13:32:40**: The previous commit added `e.preventDefault()` and `e.stopPropagation()` at the top of `onKeyDownHandler`. This commit **commented out the throttle mechanism (`if (now - lastTs < THROTTLE_MS) return;`)**, which could lead to more frequent key event processing. It also re-enabled `e.preventDefault()` and `e.stopPropagation()`. The `dispatchRewindEvent()` and `dispatchFastForwardEvent()` calls were commented out entirely from the platform-specific `switch` statements and their definitions removed from the file. This suggests a change in how rewind/fast-forward events are handled, possibly moved elsewhere or deprecated.

*   **`c:\Users\Usuario\Documents\Trabajo\MEDIAPRO\3cat_smarttv\src\components\sideMenu\index.jsx` (2/2/2026, 17:36:09 & 17:36:22)**:
    *   **17:36:09**: Major refactoring of side menu logic:
        *   Introduced `AppMenuCreate` to initialize `window.AppMenu.vars`.
        *   Added `attachSearchAtTheBeginning` to dynamically add a search menu item.
        *   Refined `refreshSelectedOption` to handle menu item selection and body class updates based on the current path.
        *   Extended menu state variables (`show_1st_mnu`, `show_2nd_mnu`, `navigate_2nd_mnu`, `selL1`, `selL2`, `language`).
        *   Updated `useEffect` hooks for menu data generation (`generateL1MenuData`, `generateL2MenuData_Explora`, `generateL2MenuData_User`) and menu state management.
        *   Added `openLevel1Menu`, `closeLevel1Menu`, `openLevel2Menu`, `closeLevel2Menu`, `exitMenu`, `exitExploraMenuRightSide`, `leave2ndLevel`, `manageFocused` functions for controlling menu visibility and focus.
        *   Updated `navigateTo` function to handle route navigation, `pageState` management, and dispatch `menu:navigate` events.
        *   Introduced `openUserMenu` and `onMouseOverUserMenu` for user menu interactions, including handling authentication states.
        *   Similarly, `openExploraMenu` and `onMouseOverExploraMenu` were added.
        *   **`generateIdFromItem` and `getSlugFromItem` callbacks were added for consistent ID generation.**
    *   **17:36:22**: A minor change, likely a re-save, where `window.__isEntry = isMenuVisible;` was uncommented in one of the `useEffect` hooks, indicating that `__isEntry` global variable is set based on menu visibility.

*   **`c:\Users\Usuario\Documents\Trabajo\MEDIAPRO\3cat_smarttv\src\views\onboarding\usersScreen\index.jsx` (2/2/2026, multiple timestamps)**:
    *   Multiple commits indicate active development and testing of `UsersScreen`.
    *   **17:45:11**: Introduced `MAX_USERS_DISPLAYED` constant. Added `onSelectUser`, `onSelectKids`, `onSelectAddUser` callbacks for user interactions. The `PageHead` `onBackClick` was conditionally set based on `window.__isEntry`.
    *   **18:04:11**: Changed `onBackClick` prop of `PageHead` to unconditionally call `window.AppGlobal.Actions.openExitAppModal`.
    *   **18:04:19**: Uncommented `if (isLoading) exitKidMode();` in `useEffect`, suggesting a bug fix or feature related to exiting kid mode when the screen is loading.
    *   **18:05:33**: Explicitly set `window.__isEntry = true;` overriding `location.state?.isEntry`, suggesting a forced entry point for this screen.
    *   **18:06:13**: Commented out `location = useLocation(); window.__isEntry = location.state?.isEntry || false;`, further simplifying the `__isEntry` logic.
    *   **18:10:38**: Reverted the change from 18:06:13, keeping `location = useLocation();` and `window.__isEntry = true;`. This indicates that `__isEntry` is always `true` for `UsersScreen` regardless of `location.state`.
    *   **18:14:59**: Introduced `window.__foceEntry = true;` with a cleanup in another `useEffect`, possibly to distinguish a "forced entry" from a standard `__isEntry` for back navigation logic.

*   **`c:\Users\Usuario\Documents\Trabajo\MEDIAPRO\3cat_smarttv\src\utils\goBackHistory.js` (2/2/2026, 18:15:16)**:
    *   Updated the `goBackHistory` function to check for `window.__foceEntry` in addition to `window.__isEntry` when deciding whether to call `callBackFunc()` (which opens the exit app modal). This aligns with the changes in `usersScreen`.

*   **`c:\Users\Usuario\Documents\Trabajo\MEDIAPRO\3cat_smarttv\src\App.jsx` (2/2/2026, multiple timestamps)**:
    *   Multiple changes related to `ExitAppModal` and `AppMenu` interactions.
    *   **18:51:35**: Commented out setting `window.isUserModalVisible = true;` and `nav.storedFocus = nav.getCurrentId(); setExitModal(ModalTypes.EXIT);` within `openExitAppModal`. This means calling `openExitAppModal` would only trigger `AppMenu?.exitMenu?.()` but not show the exit modal.
    *   **18:52:39**: Re-enabled the `window.isUserModalVisible = true; nav.storedFocus = nav.getCurrentId(); setExitModal(ModalTypes.EXIT);` lines within `openExitAppModal`, indicating the exit modal should now be displayed correctly. Additionally, `window.AppMenu.manageFocused?.()` was added to `openExitAppModal`.
    *   **18:54:07**: The lines that were previously re-enabled at 18:52:39 were moved slightly later in the `openExitAppModal` function, coming after `window.AppMenu?.exitMenu?.()`.
    *   **19:14:36**: `AppMenu.manageFocused?.()` was removed from `openExitAppModal`.
...
*   **`c:\Users\Usuario\Documents\Trabajo\MEDIAPRO\3cat_smarttv\build_templates\tizen\3cat_pro\config.xml` (3/2/2026, 13:24:24 & 5/2/2026, 10:47:12 & 11:41:17)**:
    *   **3/2/2026, 13:24:24**: Initial log entry for this file, sets Tizen widget ID, version, access origin, app control, application ID, features, metadata (including `prelaunch.support`, `use.network`, `devel.api.version`, `use.preview` endpoint, `search.format`), name, privileges, profile, and settings. The `id` is `http://tizen.tv3.ccma.cat.qa` and `application id` is `PzkKxRdFT6.TV3.qa`.
    *   **5/2/2026, 10:47:12**: Updated the `id` from `http://tizen.tv3.ccma.cat.qa` to `http://tizen.qa.ccma.cat` and the `application id` from `PzkKxRdFT6.TV3.qa` to `PzkKxRdFT6qa.TV3`. This is a change in the package and widget identification, possibly for a QA environment.
    *   **5/2/2026, 11:41:17**: No functional changes, likely a re-save or minor formatting adjustment.

*   **`c:\Users\Usuario\Documents\Trabajo\MEDIAPRO\3cat_smarttv\src\components\items\rowItems\roundItem\index.jsx` (4/2/2026, multiple timestamps)**:
    *   **11:36:24**: Initial commit shows `isKidsProfile` parameter being considered for image selection (`img.image_format === 'round'`). `className` and `style` attributes are conditional on `type` being `'round'` or `'kids-round'`. `filter: 'invert(100%)'` is applied for `logo` type images.
    *   **11:41:47**: Uncommented `if (isKidsProfile) { ... } else { ... }` block, re-enabling conditional image format logic for kids profiles (looking for `round` format for kids). The previous version had the `isKidsProfile` block commented out.
    *   **12:08:06**: Changed the image format looked for in `isKidsProfile` mode from `'round'` to `'big-round'`. This suggests a dedicated image format for kids' round items.
    *   **12:21:39**: Removed the entire `if (isKidsProfile) { ... } else { ... }` block, simplifying the image selection to always look for `round` then `logo`. The `isKidsProfile` prop is still received but no longer directly influences image selection here. The `className` and `style` for `round-image` also simplified to only check `type === 'round'`.
    *   **12:56:53**: Removed `isKidsProfile` prop from the component.
    *   **13:44:19**: Reintroduced `isKidsProfile` prop to the component, but the image selection logic inside `useMemo` remains simplified (not using `isKidsProfile` to determine image format).
    *   **13:44:59**: Replaced `isKidsProfile` with `isBigRound` as a prop influencing image format selection (looking for `'round'` format for `big-round` type images).
    *   **13:45:13**: Changed the `className` for `round-image` to `(type === 'round' || type === 'big-round') ? 'round-image' : ''`. Also `className` and `style` within `<td>` for the image was changed from `(type === 'round' || type === 'kids-round')` to `(type === 'round' || type === 'big-round')`.
    *   **13:45:24**: Minor change to the image `className` and `style` conditions, correcting a typo `'-round'` to `'big-round'`.
    *   **13:48:20**: In `useMemo`, `isBigRound` now checks for `image_format === 'big-round'` when `isBigRound` is true, indicating specific support for a "big-round" image format.
    *   **14:01:29**: Added `data-nav-fixed-focus-shape={isBigRound ? 'bigRound' : 'round'}` to the `div.roundElement`, enabling distinct focus shapes for regular round items and big round items.

*   **`c:\Users\Usuario\Documents\Trabajo\MEDIAPRO\3cat_smarttv\src\components\rows\rowUtils.js` (4/2/2026, 12:01:27 & 12:01:36)**:
    *   **12:01:36**: Corrected a typo in `getRowkind` function, changing `rowData.format === 'square-mini'` to `rowData.format === 'square_mini'`.

*   **`c:\Users\Usuario\Documents\Trabajo\MEDIAPRO\3cat_smarttv\src\components\rows\genericRow\index.jsx` (4/2/2026, 12:02:26, 12:49:54 & 12:54:35)**:
    *   **12:02:26**: Added `rowType` to `item` prop passed to `RowElement`. `onFocusAction` now includes a `console.warn` if not provided. Conditional `dataBlockFocusUp` is added for `isFirstRow`.
    *   **12:49:54**: Modified `getRowElement` to map both `'SQUARE'` and `'SQUARE_MINI'` to `SquareItem`, and `getShowAllElementsFocusShape` to map both to `'square'`.
    *   **12:54:35**: Removed a line `isKidsProfile && <div className="roundItemTitle">{item.title}</div>` from the render, moving the display of `roundItemTitle` elsewhere (likely to the `RoundRow` component itself).

*   **`c:\Users\Usuario\Documents\Trabajo\MEDIAPRO\3cat_smarttv\src\components\items\rowItems\roundItem\styles.kids.scss` (4/2/2026, multiple timestamps)**:
    *   **12:22:36**: Initial styling for `.roundElement` in `.kid-mode`, setting `background-color` to `$base-color-3cat-kid-bright-purple`.
    *   **12:22:48**: Added `border: 5px solid $base-color-3cat-kid-bright-purple !important;` to `.roundElement`.
    *   **12:23:43**: Same as 12:22:48.
    *   **12:25:30**: Removed the `border` property, leaving only `background-color`.
    *   **13:54:47**: Re-added `border: 5px solid $base-color-3cat-kid-bright-purple !important;`.

*   **`c:\Users\Usuario\Documents\Trabajo\MEDIAPRO\3cat_smarttv\src\components\fixedFocus\fixedFocus.kids.scss` (4/2/2026, multiple timestamps)**:
    *   **12:26:49**: Defined styles for `.fixed-focus` and its `.inner` for `kid-mode`, including `box-shadow` and `border-color` using `$base-color-3cat-kid-light-purple`. Also had a commented-out `&.round` block.
    *   **12:33:20**: Removed `_outline`, `border: 0px`, and `outline: 0px` from `.inner`, simplifying styling.
    *   **12:35:24**: Reverted back to the state at 12:26:49 (re-added `_outline`, `border: 0px`, `outline: 0px`).
    *   **12:35:49**: Again removed `_outline`, `border: 0px`, `outline: 0px`.
    *   **12:37:39**: Added `outline: 3px solid #be2649;` to `.inner` and uncommented the `&.round` block.
    *   **12:37:59**: Changed `outline` for `.inner` to `$base-color-3cat-kid-light-purple` and set `outline: 0px;` for `&.round .inner`.
    *   **12:42:40**: Reverted the changes, back to the state at 12:37:39, but changed `outline` to `$base-color-3cat-kid-light-purple`.
    *   **12:43:30**: Reverted to the state at 12:26:49.
    *   **12:44:18**: Similar to 12:37:59, changed `.inner` `outline` and `.round .inner` `outline`.
    *   **14:02:51**: Set `outline: 3px solid $base-color-3cat-kid-light-purple;` for `.fixed-focus .inner`.
    *   **14:06:31**: Removed the `outline` property for `.fixed-focus .inner`.
    *   **14:14:22**: Reintroduced `outline: 3px solid $base-color-3cat-kid-light-purple;` for `.fixed-focus .inner`, and added `&.round, &.big-round { .inner { outline: 0px; } }`.
    *   **14:14:40**: Changed `&.big-round` to `&.bigRound`. This sequence of changes indicates extensive fine-tuning of focus styles, especially for kid mode.

*   **`c:\Users\Usuario\Documents\Trabajo\MEDIAPRO\3cat_smarttv\src\components\rows\roundRow\index.jsx` (4/2/2026, multiple timestamps)**:
    *   **12:27:53**: Added `styles.kids.scss` import. The `roundItemWrapper` style `textAlign` is conditional on `isKidsProfile`.
    *   **12:30:58**: Removed `styles.kids.scss` import and the conditional `textAlign` based on `isKidsProfile`.
    *   **13:35:58**: Reintroduced `styles.kids.scss` import. `isKidsProfile` prop was replaced by `isBigRound`, which now dictates `textAlign` and the display of `roundItemTitle`.
    *   **13:38:35**: The main `div`'s `className` is now conditional on `isBigRound` (either `biground_row` or `round_row`).
...
*   **`c:\Users\Usuario\Documents\Trabajo\MEDIAPRO\3cat_smarttv\src\components\rows\roundRow\styles.scss` (4/2/2026, 12:28:04 & 13:40:37)**:
    *   **12:28:04**: Defines styles for `.round_row`, including `padding-left`, `height`, `width`, `margin-bottom`, and `.roundItemWrapper`.
    *   **13:40:37**: Introduced `.biground_row` with a different `height` and merged common styles for `round_row` and `biground_row`. Also added styles for `.roundItemTitle`.

*   **`c:\Users\Usuario\Documents\Trabajo\MEDIAPRO\3cat_smarttv\src\components\rows\roundRow\styles.kids.scss` (4/2/2026, 12:30:18)**:
    *   Initial empty placeholder `/* .round_row { height: 200px + 171px; } */` in `.kid-mode`.

*   **`c:\Users\Usuario\Documents\Trabajo\MEDIAPRO\3cat_smarttv\src\components\rows\genericRow\styles.scss` (4/2/2026, 12:48:42)**:
    *   Added `&.rowType_SQUARE_MINI` with a `roundItemTitle` style.
    *   Adjusted `height` and other properties for various `rowType_` classes (`LANDSCAPE`, `LIVE_TV`, `USER_ROW`, `SQUARE`, `PORTRAIT`, `PORTRAIT_MINI`, `LANDSCAPE_MINI`, `INLINE`, `FEATURED`).
    *   Updated `item-show-all` styles to be adaptive to different row types (`LANDSCAPE`, `PORTRAIT`, `SQUARE`).

*   **`c:\Users\Usuario\Documents\Trabajo\MEDIAPRO\3cat_smarttv\src\views\page\index.jsx` (4/2/2026, 13:25:55, 13:26:44 & 13:34:44)**:
    *   **13:25:55**:
        *   Added `isRankingPage` state.
        *   `filterChildren` function updated with more conditions for filtering rows based on `isLiveSupportedRef`, `isShakaPlayerSupportedRef`, `hbbtv` platform, and `activeUser?.type === UserType.KID`.
        *   `getHomeData` and `getPageData` now set `isRankingPage`.
        *   `useEffect` hooks now include `onReset` function triggered by `evChangeUser` event, which performs various state resets and reloads data.
    *   **13:26:44**: No functional change, likely re-save.
    *   **13:34:44**: No functional change, likely re-save.

*   **`c:\Users\Usuario\Documents\Trabajo\MEDIAPRO\3cat_smarttv\src\components\fixedFocus\fixedFocus.scss` (4/2/2026, multiple timestamps)**:
    *   **14:02:04**: Introduced `&.bigRound` selector with specific `width`, `height`, `border-radius`, `box-shadow`, and `border`.
    *   **14:02:35**: Updated `bigRound` to use `$base-color-3cat-kid-light-purple` for `box-shadow` and `border`.
    *   **14:03:39**: Same as 14:02:35.
    *   **14:09:26**: Added `&.round, &.bigRound` to the `.page` context for `fixed-focus` top and left positions.
    *   **14:26:06**: Changed `box-shadow` and `border` for `&.bigRound .inner` to use `$base-color-3cat-kid-white`.
    *   **14:26:27 & 14:26:32**: Added `!important` to `box-shadow` and `border` for `&.bigRound .inner` in `fixedFocus.scss`.

*   **`c:\Users\Usuario\Documents\Trabajo\MEDIAPRO\3cat_smarttv\src\App.kids.scss` (4/2/2026, 14:18:09)**:
    *   Added styling for `.V-SCROLLABLE:not(.fixed) .focused` in `kid-mode`, setting `box-shadow`, `border-color`, and `outline` to `$base-color-3cat-kid-light-purple`.

*   **`c:\Users\Usuario\Documents\Trabajo\MEDIAPRO\3cat_smarttv\src\components\modals\externalContentModal\index.jsx` (4/2/2026, 15:12:08 & 15:15:17 & 15:17:01)**:
    *   **15:12:08**: New component for displaying external content information. It takes `item`, `restoreFocusTo`, `onClose`, `onOpenExternal` props. It uses `PopupInfoGroup`, `GenericButton`, and `ImageCropper`. It has logic to display promo and platform text and dynamically split platform text by ".".
    *   **15:15:17**: Added conditional class `partner` to `popupLeft` div based on `isPartner` flag (`item.collection === 'partner'`).
    *   **15:17:01**: Modified `generateImageData` call to dynamically set `h` (height) based on `isPartner` (`428` for partner, `544` otherwise).

*   **`c:\Users\Usuario\Documents\Trabajo\MEDIAPRO\3cat_smarttv\src\components\modals\externalContentModal\styles.scss` (4/2/2026, 15:15:02)**:
    *   New styles for `.externalModalOverlay`, `.externalPopup`, `.popupLeft`, `.popupRight`, `.promoTextRow`, `.platformTextRow`, and `.popupButtons`.
    *   Includes specific sizing and layout for the modal, image containers, text, and buttons.
    *   Added `.popupLeft.partner` style to set `height: 428px;`.

*   **`c:\Users\Usuario\Documents\Trabajo\MEDIAPRO\3cat_smarttv\src\libs\img.js` (5/2/2026, multiple timestamps)**:
    *   **9:38:25**: Defined `IMGPROXY_PRESETS` and `getImgProxyUrl`. `NOT_ALLOWED_FORMATS` array included `round` twice. `getBestImageV2` has logic to filter images based on `desiredAspect` and `NOT_ALLOWED_FORMATS`.
    *   **9:38:51**: Added `"square-round"` to `notAllowedFomats.push(...)` in `getBestImageV2` for landscape aspect.
    *   **9:40:12**: Corrected `NOT_ALLOWED_FORMATS` by removing the duplicate `"round"` entry, but added it back to `getBestImageV2` in `notAllowedFomats.push(...)`.
    *   **9:40:33**: Corrected `NOT_ALLOWED_FORMATS` to only have "round" once in the main array, but then adds "square-round" to `notAllowedFomats` in `getBestImageV2`.
    *   **9:44:04**: Added `"big-round"` to `notAllowedFomats.push(...)` for landscape aspect in `getBestImageV2`. This indicates the introduction of a new image format to be excluded for certain aspect ratios.
    *   **9:46:50 & 9:46:57**: No functional changes, likely re-saves.

### Patterns and Recurring Elements:

*   **Kid Mode (`.kid-mode`, `isKidsProfile`, `isBigRound`)**: A significant amount of work is dedicated to styling and logic for a "kid mode" or "Super3" profile. This includes:
    *   Specific `styles.kids.scss` files for components like `roundItem`, `fixedFocus`, and `App`.
    *   Conditional rendering and styling based on `isKidsProfile` and `isBigRound` props.
    *   Special image formats (`big-round`) are introduced and handled.
    *   The `usersScreen` also specifically supports a "Super3" (kid) profile.
    *   The `Page` component filters out `user_row` content for kid users and modifies home screen data based on kid profiles.
*   **Focus Management (`nav.focusById`, `registerFocusAction`, `unregisterFocusAction`, `data-block-focus-*`, `data-on-lost-left-action`, `fixedFocus`)**:
    *   Extensive use of a custom navigation/focus library (`nav`).
    *   Consistent patterns of `registerFocusAction` and `unregisterFocusAction` in `useEffect` hooks across various item components (`RoundItem`, `GenericRow`, `ShowAllElements`).
    *   Attributes like `data-group`, `data-group-restore-last-focused`, `data-nav-fixed-focus-shape`, `data-block-focus-up/right/left/down`, and `data-on-lost-left-action` are heavily used to control navigation behavior in the Smart TV UI.
    *   The `fixedFocus` component and its `fixedFocus.scss` and `fixedFocus.kids.scss` files are frequently updated to refine visual focus feedback for different item shapes and modes.
*   **Image Handling (`getImgProxyUrl`, `IMGPROXY_PRESETS`, `getBestImage`, `removeItemsFromArray`, `generateImageData`)**:
    *   Centralized image processing in `src\libs\img.js` using `IMGPROXY_URL` and predefined presets for different resolutions (`xs` to `xl`).
    *   Logic for selecting the "best" image based on dimensions, aspect ratio, and priority (text/no-text).
    *   Explicitly defined `NOT_ALLOWED_FORMATS` for images, which is being constantly refined. The `getBestImageV2` function has more complex filtering based on desired aspect ratio and additional "not allowed" formats.
*   **Platform Specifics (`window.AppGlobal.platform`, `webOS.service.request`, `tizen:`)**:
    *   The `gateway\js\app.js` and `platform\generic.js` files show clear distinctions in handling WebOS and Tizen platform-specific services and key codes.
    *   The `tizen\config.xml` file is specific to Tizen applications, defining app ID, privileges, and metadata.
*   **Modals and User Experience**:
    *   Frequent adjustments to modal visibility and focus management (`ExitAppModal`, `UserModal`, `ExternalContentModal`).
    *   The `goBackHistory` utility function is crucial for consistent back navigation across different screens and modal states.
    *   `UsersScreen` and `SideMenu` updates indicate a focus on user authentication, profile switching (including kid mode), and overall app navigation flow.
*   **Minor Fixes/Refinements**: Many small adjustments like typo corrections (`square-mini` to `square_mini`), commenting/uncommenting debug lines, and re-enabling/disabling specific logic indicate ongoing development, testing, and fine-tuning.